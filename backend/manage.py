...
@click.group()
def manage():
    '''This script manages the backend project.'''
...
def runserver(port):
    ...
    cmd = ''
    if os.name == 'posix':
        cmd = f'. .venv/bin/activate; export FLASK_APP=backend.app; export FLASK_ENV=development; flask run --port={port}'
    elif os.name == 'nt':
        cmd = f'call .venv/Scripts/activate; set FLASK_APP=backend.app; set FLASK_ENV=development; flask run --port={port}'
...
def plug_blueprint(name: str, templates: bool):
    ...
    init = open(os.path.join(
        os.getcwd(), 'backend', 'blueprints', name, '__init__.py'), 'w')
    init.close()
    with open(os.path.join(os.getcwd(), 'backend', 'blueprints', name, f'{name}.py'), 'w') as blueprint:
    ...
    settings['default']['EXTENSIONS'].append(
        f'backend.blueprints.{name}.{name}:init_app')
...
def plug_database():
    ...
    settings = toml.load(os.path.join(
        os.getcwd(), 'instance', 'settings.toml'))
    if 'backend.ext.database:init_app' in settings['default']['EXTENSIONS']:
    ...
    with open(os.path.join(os.getcwd(), 'backend', 'ext', 'database.py'), 'w') as db_module:
    ...
    cmd = ''
    if os.name == 'posix':
        cmd = f'. .venv/bin/activate; export FLASK_APP=backend.app; export FLASK_ENV=development; flask db init'
    elif os.name == 'nt':
        cmd = f'call .venv/Scripts/activate; set FLASK_APP=backend.app; set FLASK_ENV=development; flask db init'
    subprocess.run(cmd, shell=True)
...
def db_migrate(message):
    settings = toml.load(os.path.join(
        os.getcwd(), 'instance', 'settings.toml'))
    if 'backend.ext.database:init_app' not in settings['default']['EXTENSIONS']:
